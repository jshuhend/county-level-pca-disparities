...
knitr::opts_knit$set(root.dir = "C:/Users/jshuhend/OneDrive - Fred Hutch Cancer Center/Projects/PCa-Disparities/Data")
...
read_and_merge <- function(data) {
  ...
  dset$variance_ratio_diff <- dset$variance_ratio_old + dset$variance_ratio_new
...
|psa era mortality ratio - pre psa era mortality ratio| = screening frequency

```{r}
# how to weight?
rdset <- select(dset, diff, screening_frequency, variance_ratio_diff)
rdset$screening_frequency <- rdset$screening_frequency / 10
rdset <- na.omit(rdset)

model_diff <- lm(diff ~ screening_frequency,
                 data = rdset,
                 weights = 1 / variance_ratio_diff)
...
```{r}
??
riset <- select(dset, screening_frequency, white_rate_old, white_rate_new, black_rate_old, black_rate_new, fips)
riset$screening_frequency <- riset$screening_frequency / 10
#riset <- na.omit(riset)

riset <- data.frame(
  fips = rep(riset$fips, 2),                                 
  screening_frequency = rep(riset$screening_frequency, 2),   
  time = rep(c(0, 1), each = nrow(riset)),         
  white_rate = c(riset$white_rate_old, riset$white_rate_new), 
  black_rate = c(riset$black_rate_old, riset$black_rate_new)  
)

riset$ratio <- riset$black_rate / riset$white_rate 


model_interaction <- lm(ratio ~ screening_frequency + time + screening_frequency * time,
                 data = riset)#,
                 #weights = 1 / )

t4 <- tbl_regression(model_interaction)

t4
```

```{r}
coef(summary(model_interaction))
```

```{r}
car::deltaMethod(model_interaction, "screening_frequency + time + `screening_frequency:time`")
```
...
shows which counties we have data for. Using all years of screening and mortality data from two periods
...
count_table <- function(data){
  ...
  bset <- filter(bset, Year != 2020)
  bset <- filter(bset, Response != "No")
  bset <- select(bset, Year, Locationabbr, Data_value)  
  colnames(bset) <- c("Year", "cbsa", "Screening Frequency")
  bset <- pivot_wider(bset, names_from = Year, values_from = "Screening Frequency", names_prefix = "Screening Frequency ")
  bset <- bset[, c(1, ncol(bset):2)]
  ...
  tset <- full_join(bset, cset, by = "cbsa")
  tset <- full_join(tset, mset1, by = "fips")
  tset <- full_join(tset, mset2, by = "fips")
  
  return(tset)
}

tset <- count_table(data)

write.csv(tset, "C:/Users/jshuhend/OneDrive - Fred Hutch Cancer Center/Projects/PCa-Disparities/Output/pca_psa_data_table.csv")
